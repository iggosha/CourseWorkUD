...
public class PlantsController {
    ...
    @FXML
    private TableView<ObservableList<String>> plantsTable;
    ...
    private void fillUpdatesTextField() {
        StringBuilder stringBuilder = new StringBuilder(plantsTable.getSelectionModel().getSelectedItem().toString());
    ...
    private void fillOrderByComboBox() {
        ...
        for (TableColumn<ObservableList<String>, ?> tc : plantsTable.getColumns()) {
    ...
    private void deleteRow() {
        ...
        Label userLabel = new Label("Вы уверены, что хотите удалить запись "
                + plantsTable.getSelectionModel().getSelectedItem() + "?");
        ...
        if (option.get() == enterYes) {
            String sqlQuery = "DELETE FROM " + nameLabel.getText() + " WHERE " +
                    plantsTable.getColumns().get(0).getText() + " = " + plantsTable.getSelectionModel().getSelectedItem().get(0);
    ...
    private void updateRow() {
        ...
        for (int i = 0; i < plantsTable.getColumns().size(); i++) {
            stringBuilderValues
                    .append(plantsTable.getColumns().get(i).getText())
                    .append(" = ")
                    .append(oldValues[i])
                    .append(", ");
        }
        ...
        stringBuilderValues
                ...
                .append(" WHERE ")
                .append(plantsTable.getColumns().get(0).getText())
                .append(" = ")
                .append(plantsTable.getSelectionModel().getSelectedItem().get(0))
                .append(";");
    ...
    private void makeTableView() {
        plantsTable.getColumns().clear();
        String sqlQuery = "SELECT * FROM " + nameLabel.getText();
        sqlQuery = utilsController.appendWhereAndOrderByToQuery(whereTextField, orderByComboBox, ascCheckBox, sqlQuery);
        utilsController.fillTableWithSqlQuery(plantsTable, sqlQuery);
    }
